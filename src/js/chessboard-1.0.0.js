  ...
  function constructor (containerElOrString, config) {
    ...
    function buildSparePiecesHTML (color) {
      ...
      for (var i = 0; i < pieces.length; i++) {
        if(i == 0){
          html += buildPieceHTML(pieces[i], true, sparePiecesElsIds[pieces[i]])
        }else{
    ...
    function initDOM () {
      ...
      if (config.sparePieces) {
        $sparePiecesTop = $container.find('.' + CSS.sparePiecesTop).attr('id', 'sparePiecesTop')
        $sparePiecesBottom = $container.find('.' + CSS.sparePiecesBottom).attr('id', 'sparePiecesBot')
      }
